...
# === PENCARIAN AHO-CORASICK ===
def search_keywords_aho_corasick(keywords, files, applicant_db, cv_mapping):
    """Search keywords using Aho-Corasick algorithm"""
    result = []
    keywords_lower = [kw.strip().lower() for kw in keywords if kw.strip()]
    
    print(f"🔍 Searching for {len(keywords_lower)} keywords using Aho-Corasick: {', '.join(keywords_lower)}")
    print(f"📄 Scanning {len(files)} CV files...")
    
    for filename, text in files:
        text_lower = text.lower()
        
        # Use Aho-Corasick to find all patterns
    ...
    print(f"🎯 Found {len(result)} CVs containing keywords")
...
# === FILTER RESULTS BY LOGIC ===
def filter_results_by_logic(results, keywords, logic):
    """Filter results based on AND/OR logic"""
    if logic == 'and':
        # AND logic: must contain ALL keywords
        filtered = [r for r in results if r["keywords_found"] == len(keywords)]
        print(f"🔗 AND logic applied: {len(filtered)} CVs contain ALL keywords")
        return filtered
    else:
        # OR logic: can contain ANY keyword (already satisfied by Aho-Corasick results)
        print(f"🔗 OR logic applied: {len(results)} CVs contain ANY keyword")
        return results
...
# === MAIN ===
def main():
    print("🚀 Initializing CV Search System (Aho-Corasick)...")
    ...
    print("\n=== Aho-Corasick CV Keyword Search ===")
    print("💡 Tips:")
    print("   - Single keyword: python")
    print("   - Multiple keywords (comma): react, express, html")
    print("   - Multiple keywords (spaces): react express html")
    print("   - Aho-Corasick excels at finding multiple patterns simultaneously!")
    ...
    while True:
        ...
        all_results = search_keywords_aho_corasick(keywords, files, applicant_db, cv_mapping)
        
        # Filter results based on logic for multiple keywords
        if len(keywords) > 1:
            results = filter_results_by_logic(all_results, keywords, search_logic)
        else:
            results = all_results

        duration = time.time() - start
        print(f"\n✅ Aho-Corasick Search completed in {duration:.3f}s ({int(duration * 1000)}ms)")
    ...
    print("\n👋 Terima kasih telah menggunakan CV Search System (Aho-Corasick)!")
